...
int main_tp12(int argc, const char * argv[])
{
    ...
    printf("main_tp12():...\n");
    char filename[]="Users/Plane Master/CLionProjects/LP1/data/rects.txt";
...
// estas duas funções facilitam a vida... evita repetir código
...
void insert_rect_dyn_array_rects(DYNARRAYRECTS dar, RECTANGLE rect)
{
    RECTANGLE *prect = dar.prects, r0={{0,0},{0,0}};

    //Look for empty position and insert rect
    for(int i=0; i<dar.size;i++)
    {
       if(rectanle_compare(r0,*prect)){ //OU rectanle_compare(r0,*(prect+i));
           (*prect) = rect; //OU *(prect+i) = rect;
           return;
       }
       prect++;
    }
...
void read_dyn_array_rects_txt(DYNARRAYRECTS *pcs, char filename[])
{
    ...
    fscanf(fp, "%*s %d", &size);
    printf("save_dyn_array_rects_txt(): size = %d\n", size);
    ...
    for(int i=0;i<pcs->size;i++)
    {
        ...
        // ii)  ignore ':'                          -> %*[:]
        fscanf(fp, "%[^:] %*[:] %d %d %d %d",&(prect->downpt.x),&(prect->downpt.y), &(prect->uppt.x), &(prect->uppt.y));
        prect++;
        //OU:
        //fscanf(fp, "%[^:] %*[:] %d %d %d %d",&(r.downpt.x),&(r.downpt.y), &(r.uppt.x), &(r.uppt.y) );
        //insert_rect_dyn_array_rects(*pcs, r);
    }
